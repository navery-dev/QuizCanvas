...
# Add this to your backend/quizapp/views.py temporarily for debugging
...
def test_gmail_connection(request):
    ...
    try:
        ...
        # Show current Django email settings
        config = {
        ...
        try:
            ...
            config['connection_test'] = f'FAILED: {str(e)}'
            return APIResponse.error('Gmail connection failed', data=config)
        
        # Test 2: Send actual test email
        try:
            test_email = settings.EMAIL_HOST_USER  # Send to yourself for testing
            send_mail(
                subject='QuizCanvas Gmail Test',
                message='This is a test email from QuizCanvas to verify Gmail configuration.',
                from_email=settings.DEFAULT_FROM_EMAIL,
                recipient_list=[test_email],
                fail_silently=False
            )
            config['test_email_sent'] = f'SUCCESS to {test_email}'
            
        except Exception as e:
            config['test_email_sent'] = f'FAILED: {str(e)}'
            return APIResponse.error('Gmail test email failed', data=config)
        
        return APIResponse.success('Gmail tests passed!', data=config)
        
    except Exception as e:
        return APIResponse.error(f'Gmail debug test failed: {str(e)}')
